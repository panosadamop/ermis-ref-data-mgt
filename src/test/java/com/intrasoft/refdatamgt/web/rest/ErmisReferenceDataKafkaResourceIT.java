...
import com.intrasoft.refdatamgt.config.KafkaProperties;
...
class ErmisReferenceDataKafkaResourceIT {
    ...
    void setup() {
        ...
        ErmisReferenceDataKafkaResource kafkaResource = new ErmisReferenceDataKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ermis-reference-data-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ermis-reference-data-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
